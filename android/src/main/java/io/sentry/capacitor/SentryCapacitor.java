...
public class SentryCapacitor extends Plugin {
  ...
  public void initNativeSdk(final PluginCall call) {
    ...
    SentryAndroid.init(
      ...
      options -> {
        ...
        if (capOptions.has(("nativeTracePropagationTargets"))) {
          try {
            List<String> targets = GetStringArray(capOptions.getJSONArray("nativeTracePropagationTargets"));
            options.setTracePropagationTargets(targets);
          } catch (Exception ex) {
            String errorMessage = ex.getMessage(); // Use getMessage() for a concise error description
            logger.warning("Failed to propagate tracePropagationTargets to the Native SDK. Error: " + errorMessage);          }
        }
  ...
  private List<String> GetStringArray(JSONArray array) throws JSONException {
